            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 lg:p-8 bg-white dark:bg-gray-800 dark:bg-gradient-to-bl dark:from-gray-700/50 dark:via-transparent border-b border-gray-200 dark:border-gray-700">
                    @foreach($division->circulars as $circular)
                        <h4 class="mt-2  text-1xl font-medium ">
                            <span class="font-bold text-black ">Date: {{\Carbon\Carbon::parse($circular->created_at)->format('d-M-Y H:i:s')}} </span>
                            <br>
                            <a href="{{\Illuminate\Support\Facades\Storage::url($circular->document)}}" class="hover:underline text-blue-500" target="_blank">
                                {{$circular->circular_no}} - {{ $circular->title }} - By {{ \App\Models\User::find($circular->user_id)->name }}
                            </a>
                        </h4>
