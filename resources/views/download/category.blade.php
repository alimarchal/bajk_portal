                    ...
                    @foreach($downloads as $manual)
                        {{$loop->iteration }} - <a href="javascript:;" class="text-blue-500 hover:underline">
                            ...
                            <a href="{{\Illuminate\Support\Facades\Storage::url($manual->document)}}" target="_blank"
                               class="text-red-500 font-extrabold  hover:underline">[Download]</a>
                        </a>

                        @can('delete')
                            <form action="{{ route('download.destroy', $manual->id) }}" method="POST"
                        ...
                        @endcan<!-- Delete button -->

                        <p class="mt-2 mb-2 text-black leading-relaxed">
                            {{ Str::limit(strip_tags($manual->description), 200)}}
                        </p>
